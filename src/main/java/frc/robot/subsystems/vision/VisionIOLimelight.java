...
public class VisionIOLimelight implements VisionIO {
  ...
  public VisionIOLimelight(
      Transform3d cameraPose, String name, Supplier<Rotation2d> rotationSupplier) {
    ...
    new Trigger(DriverStation::isEnabled)
        .onTrue(Commands.runOnce(() -> setImuMode(1)))
        .onFalse(Commands.runOnce(() -> setImuMode(0)));
  }

  /**
   * Sets the IMU mode for the LL4
   *
   * @param mode 0 - Use external IMU yaw submitted via SetRobotOrientation() for MT2 localization.
   *     The internal IMU is ignored entirely. 1 - Use external IMU yaw submitted via
   *     SetRobotOrientation(), and configure the LL4 internal IMU's fused yaw to match the
   *     submitted yaw value. 2 - Use internal IMU for MT2 localization.
   */
              ...
              .filter(
                  x ->
                      x != null
                          && x.id < 23
                          && x.id > 0
                          && x.ambiguity < VisionConstants.maxAmbiguity)
